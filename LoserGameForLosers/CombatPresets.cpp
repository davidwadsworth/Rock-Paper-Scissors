...
void CombatPresets::ResetCombat::init()
{
	...
	if (player_2_->has_component<ControllerComponent>())
...
void CombatPresets::SinglePlayerSelectAttack::init()
{
	...
	auto ai_choose_attack = new PathBranch(select_attack, nullptr, clean_up);
...
void CombatPresets::SinglePlayerCombat::init()
{
	...
	disable_controller->add_navigator(new Navigation::DeactivateAI(player_2_));
	...
	enable_controller->add_navigator(new Navigation::ActivateAI(player_2_));
...
void CombatPresets::MatchEnding::init()
{
	...
	disable_controllers->add_navigator(new Navigation::StopMusic(GameState::get_audio_player()));
