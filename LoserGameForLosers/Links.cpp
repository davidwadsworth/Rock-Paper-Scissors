...
void CursorLeftLink::set_link_information(SDL_Rect * starting_rect)
{
	...
	if (starting_rect)
	{
		x_offset = static_cast<int>((link_dimensions_->w + CURSOR_WIDTH + padding_ * 6 + pos_.x - (starting_rect->w + starting_rect->x)) * alignment_.x);
		y_offset = static_cast<int>((link_dimensions_->h + padding_ * 4 + pos_.y - (starting_rect->h + starting_rect->y)) * alignment_.y);
	}
