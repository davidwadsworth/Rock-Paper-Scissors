...
namespace Navigation
{
	...
	int PushPlayer::choose_path()
	{
		if(!used_ && attacker_->get_component<ColliderComponent>().collides(victim_))
		{
			if (victim_->get_component<PlayerComponent>().attack_id != attack_block)
				...
				victim_->state->audio_player->play_sound(sound_blocking);
	...
	BlockPlayer::BlockPlayer(Entity * blocker, const Uint32 attack_frames)
		: Delay(attack_frames), blocker_(blocker), used_(false)
	{}
	...
	ReceiveDamage::ReceiveDamage(Entity * player, const int damage)
		: player_(player), damage_(damage)
	{}

	int ReceiveDamage::choose_path()
	{
		player_->get_component<PlayerComponent>().receive_damage(damage_);
		return 1;
	}

	CheckRoundLoss::CheckRoundLoss(Entity * player)
		: player_(player), match_end_(player)
	{}

	int CheckRoundLoss::choose_path()
	{
		if (player_->get_component<PlayerComponent>().check_player_knock_out())
		{
			match_end_.init();
			player_->state->path->clear();
			player_->state->path->add(match_end_.get_trunk());
			return 1;
		}
