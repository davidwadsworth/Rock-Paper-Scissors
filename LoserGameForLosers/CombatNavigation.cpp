...
namespace Navigation
{
	...
	{
		if (player_1_->get_component<ColliderComponent>().collides(player_2_, COMBAT_PADDING))
		{
	...
	void BlockAndCrawl::init()
	{
		blocker_->get_component<PlayerComponent>().choose_combat_attack(attack_block);
		blocker_->get_component<PlayerComponent>().execute_combat_attack();
		blocker_->get_component<PlayerComponent>().set_velocity_offset(velocity_offset_);
	}


	void BlockAndCrawl::close()
	{
		blocker_->get_component<PlayerComponent>().reset_character_hit_box();
		blocker_->get_component<PlayerComponent>().reset_velocity_offset();
	}
	...
	int CheckRoundLoss::choose_path()
	{
		if (overlay_->get_component<OverlayComponent>().is_depleted())
		{
			...
			for (auto p : GameState::get_manager()->get_group(Game::group_players))
				p->get_component<ScriptComponent>().clear();
	...
	{
		player_->get_component<PlayerComponent>().set_velocity_offset(scale_);
	}
	...
	void MovePlayer::close()
	{
		...
		player_->get_component<PlayerComponent>().reset_velocity_offset();
	}
	...
	int DeactivateAI::choose_path()
	{
		bot_->get_component<AIComponent>().deactivate();
		return 1;
	}

	int ActivateAI::choose_path()
	{
		bot_->get_component<AIComponent>().activate();
		return 1;
	}

	int SelectAttackAI::choose_path()
	{
		bot_->get_component<AIComponent>().select_attack();
		return 1;
	}
